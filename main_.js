...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // [新增] 處理手機版懸浮按鈕的點擊事件
  const fabButton = document.getElementById('fab-add-task-btn');
  if (fabButton) {
    fabButton.addEventListener('click', () => {
      const addTaskDiv = document.querySelector('.add-task-controls'); // [修正] class 名稱
      if (addTaskDiv) {
        addTaskDiv.scrollIntoView({ behavior: 'smooth', block: 'center' });
        setTimeout(() => {
          document.getElementById('task-template-select')?.focus();
        }, 300); // 短暫延遲後再聚焦，確保滾動動畫完成
      }
    });
  }
