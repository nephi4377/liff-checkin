# 專案計劃書：裝修工料檢核小幫手 (Renovation Material & Tool Checker) v2.3

**版本更新日期**：2025-12-16  
**狀態**：已實現核心功能（圖片搜尋、配方管理 UI 完整、GAS 後端整合）；待優化資安與 UX 改善
...
1.  **多工項清單 (Shopping Cart Mode)**  
    ...
    *   ✅ 實現位置：`renovation_checker.html` 第 1119–1127 行 (`addTask()`, `removeTask()`)

2.  **材料自動彙整**  
    *   ✅ 系統於前端記憶體自動計算所有工項的材料總和  
    *   ✅ 依 `mat_id` + `spec` 進行合併，支援損耗率與耗材標記  
    *   ✅ 實現位置：`renovation_checker.html` 第 1240–1291 行 (`calculatedMaterials` computed)

3.  **智慧工具清單**  
    *   ✅ 工具計算與分類（動力/手工）  
    *   ⚠️ **部分實現**：合併邏輯簡化，未完整實現 `alloc_rule` 與多人配置邏輯  
    *   ✅ 實現位置：`renovation_checker.html` 第 1295–1344 行 (`calculatedTools` computed)

4.  **施工/檢測要點彙整**  
    *   ✅ 依據已選工項動態產生 SOP 檢查表  
    *   ✅ 實現位置：`renovation_checker.html` 第 362–369 行 (SOP tab)
...
5.  **配方管理（CRUD）UI**  
    ...
    *   ✅ 實現位置：`renovation_checker.html` 第 823–970 行

6.  **圖片搜尋與預覽**  
    *   ✅ 使用 Google Custom Search API 搜尋材料圖片  
    *   ✅ 圖片 Modal 與切換功能（`openImageModal()`, `cycleImage()`）  
    *   ⚠️ **TODO**：圖片保存尚未寫回 Sheet（暫存於本地記憶體）  
    *   ✅ 實現位置：`renovation_checker.html` 第 985–1095 行

7.  **資料庫讀取**  
    *   ✅ 使用 **GViz JSONP** 方式平行讀取 5 張 Google Sheet  
    *   ✅ 實現位置：`renovation_checker.html` 第 1132–1169 行 (`fetchSheet()`)

### ⚠️ 待實現/已知限制
1.  **配方寫回持久化**：UI 標記 `recipeManager.hasChanges = true`，但 `saveAllRecipes()` 僅為 alert + log，**未實作寫回 Google Sheets**（第 977–982 行）  
2.  **圖片儲存**：`saveSelectedImage()` 只更新前端物件，**未將圖片 URL 與授權資訊寫回 Sheet**（第 1091–1097 行）  
3.  **匯出功能**：計劃中的「CSV 下載」、「分階段匯出」尚未實現  
4.  **複製叫料單**：`copyToClipboard()` 已實現（第 1357–1369 行），可複製文字  
