...
- v1.1 仍僅使用瀏覽器 `localStorage`，但已補上 `assigned → in_progress → reported → approved / rework` 狀態機；事件以 append-only 保存 `project_id`、操作者 `employee_id`、ISO 時間、狀態轉移與證據網址。尚未新增 GAS／Sheet 資料流，不可視為跨裝置共用的正式任務紀錄。
