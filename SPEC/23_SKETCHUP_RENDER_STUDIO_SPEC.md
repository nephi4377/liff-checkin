...
- 雙主按鈕：「只加光線質感」／「依下方設定美化」
- 燈光來源 chips＝允許的燈種（含壁燈／軌道燈／落地燈／鏡前燈／吸頂燈／其他自行描述）；未勾選「允許加燈」則不可加燈或加盞數
- **指定某處顏色**：色盤＋位置；未指定時天花與牆同色單色
- **櫃體／門片**：配色延用原圖；可選填材質名稱；門片跳色禁止亂換
- **畫筆**：燈箱放大、可調粗細／顏色、橡皮擦、上一步
- **這次送出哪一張**：可選原圖或某個渲染版本
- **下載**：渲染結果可下載目前這張，或一次下載此圖全部版本
- 後端基本 prompt：**不加櫃體內側燈、不新增櫃體、不自行加家具**
- 單張渲染／批次渲染（批次一張完成就顯示）
- 開頁連線失敗：人話說明＋「再試一次」（自動再 ping 一次）；不要只停在「載入中」或讓人以為沒開成
- 批次渲染結束：有失敗時狀態列必須是失敗（紅字），不可寫成「結束」當成功；失敗的圖可再單張渲染
...
- 開頁失敗：`?api=http://127.0.0.1:9` 應看到「載入失敗」與「再試一次」
- 單元：`node tools/sketchup-render-studio/batch-finish.test.mjs`
