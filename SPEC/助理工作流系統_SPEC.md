...
- [x] 結構化回報、附件證據、退回原因與事件時間軸。
- [ ] 我的工作、主管審核中心、逾期與等待中篩選。
  - [x] 我的工作：依執行者 `employee_id` 彙整跨工作流待辦，優先顯示可執行步驟並可切回案件。
  - [ ] 主管審核中心：依審核者 `employee_id` 彙整待審核任務。
  - [ ] 逾期與等待中篩選。
...
- 2026-09-10：P0 v1.1 補齊任務狀態操作；以 `project_id`、執行者／審核者 `employee_id` 鎖定身分，提交前檢查必填欄位與證據網址，並用 append-only 事件保留操作者、ISO 時間與狀態轉移。
