...
> 自 2026-06 起，現行流程見 [`00_開發憲法.md`](../00_開發憲法.md) 與 `.cursor/rules/`。  
> 以下保留舊版 Antigravity／多子系統 SOP，僅供日後若要恢復分工編排時查閱。
...
* **核心職責**：負責 Static Analysis、代碼語法檢查（如 `node -c`），驗證變更是否與 `專案全域資料字典.md` 的參數命名一致，並審查變更是否破壞現有的 `@STABLE` 穩定函數。
