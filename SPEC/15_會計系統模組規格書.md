...
### 2.1 收支登錄（獨立頁已停用 · 2026-07-10）

- **獨立頁停用**：`accounting_ingest.html` 改為停用說明並導向會計選單；HUB `#/accounting-ingest` 改開 `#/accounting`。
- **原因**：直接 `accounting_form_submit` 寫入收支／毛利／客戶收款，與 HUB「待審→客戶確認→入帳」不同步。
- **請改用**：`#/accounting` 會計功能選單；未付款 → 待付款申請；案件收入／收款 → 案件毛利／收款流程。
- LINE 群組記帳若仍啟用，走後端 ingest，不經此獨立網頁。
- 舊說明 `#/help/accounting-ingest` 改導向待請款說明。

（以下為歷史實作摘要，僅供對照）

- 曾有靜態頁連續記帳、HUB 快取代入、SPA 殼層內嵌
