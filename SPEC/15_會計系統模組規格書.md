# 會計系統模組規格書 (v1.2 - 雲端與本機雙軌過渡版)

本規格書定義「添心會計系統」之核心模組、資料流向與本地持久化方案。考量現行會計作業習慣與未來保密需求，系統目前處於 **Phase 2 雲端與本機雙軌過渡期**：日常記帳仍以雲端 Google Sheets 為主檔，本機 SQLite 則專責機密轉帳與進階工具，未來再逐步向 100% 不落地邁進。
...
- **資料流向與保密策略 (Phase 2)**:
  * **日常主檔 (Google Sheets)**：透過 LINE、`project-console` 與 `accounting-gas`，將日常收支明細寫入雲端 `115年X月` 試算表，供會計即時核算。
  * **本機高保密區 (SQLite)**：網銀轉帳設定、高度機密帳務、離線草稿與本機進階 OCR 結果，存放於本機 SQLite，不上傳公有雲。
  * **未來目標 (Phase 3+)**：逐步將 Sheets 轉為本機 SQLite 的唯讀鏡像，最終實現 100% 不落地的零信任防禦。
  * **Dropbox 自動備份**：本地資料庫（如 `data/accounting.db`）利用 Dropbox 同步目錄傳輸，兼顧安全與備份。
...
- **財務資料隔離**：本機專屬的機密帳務只在本地與「案號」關聯，**不會**將這部分機密金額寫回日常雲端的 Google Sheets，維持特定資料的隱密性。
...
| 階段 | 做法 |
|------|------|
| **開發測試（現行）** | GAS Script Property `ACCOUNTING_AUTH_BYPASS=true`；`AccountingApi.initSession()` 略過 LIFF，一般瀏覽器即可開頁測試 |
| **正式上線** | `ACCOUNTING_AUTH_BYPASS=false`；於會計官方 LINE Channel 建立 LIFF、寫入 `ACCOUNTING_LIFF_ID`；表單須在 LINE 內開啟並驗證 `id_token` |
