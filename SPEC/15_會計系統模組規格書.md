...
- **狀態紀錄（單頁模式）**：右側「狀態紀錄」只掛在外層殼層一次；`?embed=1` 子頁不重掛第二個面板，操作訊息以 `postMessage` 轉給外層（`accounting_ui.js`）

- **寬螢幕主內容**：主欄約視窗寬 **55–65%**（`accounting_layout.css`）；內嵌子頁主區撐滿 iframe，不重套左右留白
