...
**v1.10 更新（2026-07-18）**：主檔收斂拍板——**收款帳戶廢棄**、**費用分類不要**；審核／匯款狀態以請款申請欄位＋審核／待匯款／款項進度為準（**不做收支狀態輔助維護頁**）；訂編案號對照**暫留表、不開頁**（待最終拍板）；LINE 綁定維持廠商名冊同一頁。
...
- 狀態欄位在 **「廠商請款申請」**：`review_status`、`payment_status`；另於 `vendor_payment_finance.html`（待匯款）、`vendor_status.html`（款項進度）操作／查看

- **不做**「收支狀態輔助」維護頁；該分頁僅後端同步殘留，日常不以它為準
...
| 廠商LINE綁定 | `vendor_line_binding` | UID／GID 多筆綁定 | ✅ CRUD（畫面在廠商名冊） |

| 廠商請款申請 | `vendor_payment_request` | 待付款主流程 | ✅ 必要 |

| 請款分攤 | `vendor_payment_allocation` | 一筆款多案 | ✅ 必要 |

| 收款帳戶 | `payee` | （舊）匯款名冊 | **廢棄 2026-07-18**；API 禁寫 |

| 收支狀態輔助 | `ledger_status` | （舊）審核輔助 | **不做維護頁**；狀態以請款申請為準 |

| 費用分類 | `category` | （舊）科目表 | **不要 2026-07-18**；用程式列舉；API 禁寫 |

| 訂編案號對照 | `order_project_map` | 訂編 ↔ 案號 | **暫留表、不開頁**（待最終拍板）；bootstrap 仍載 |
