...
- **主檔快取**：進入 `index.html` 時 `accounting_bootstrap` 一次載入，子頁讀 `sessionStorage`（TTL **3 天**；有改動時 patch／背景重讀）
...
- 廠商 LINE 綁定：從 project-console「顧客列表」搜尋（`line_contact_search`），一廠商可綁多筆 UID／GID
