...
- **Update Text（施工日誌編輯文字）**：可先顯示新字、卡片半透明。**失敗必須還原舊字、拿掉半透明**，並用人話說「沒存到、請再按編輯文字再存」。不可維持新字讓人以為已經改好。
- **Create**: 產生 `temp-ID` 的卡片並加上 0.7 透明度，直到輪詢完成後透過 `replaceOptimisticCard` 替換為正式 ID 卡片。

```mermaid
flowchart TD
  start[人按儲存文字] --> preview[畫面先顯示新字、半透明]
  preview --> wait[背景送出]
  wait -->|成功| done[確認已存好]
  wait -->|失敗| rollback[還原舊字、說沒存到]
  rollback --> retry[人再按編輯文字後儲存]
```
...
| 白話（圖上寫的） | 程式對照 |
|------------------|----------|
| 人按儲存文字 | `logActions.handleSaveText` |
| 畫面先顯示新字、半透明 | 卡片 `opacity` 0.7；寫入本機快取 |
| 背景送出 | `projectApi.request` action `updateLogText` |
| 確認已存好 | 拿掉半透明＋成功提示 |
| 還原舊字、說沒存到 | `logTextRollback` 還原畫面／快取＋人話通知 |
