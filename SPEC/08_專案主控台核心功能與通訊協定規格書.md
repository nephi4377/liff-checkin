...
- **Delete**: 直接移除 DOM 並更新 LocalStorage 快取。失敗時卡片已消失，須請人刷新（尚未自動還原）。
- **Update Text**: 直接更新 DOM 文字，背景同步。
- **Create**: 產生 `temp-ID` 的卡片並加上 0.7 透明度，直到輪詢完成後透過 `replaceOptimisticCard` 替換為正式 ID 卡片。
- **Update Photos（施工日誌管理照片）**：可先用本機預覽網址顯示新圖、卡片半透明。**失敗必須還原舊圖、拿掉半透明**，並用人話說「沒存到、請再開管理照片再試」。不可維持新圖半透明、讓人以為已經改好。預覽網址必須是字串（`data:image/...` 或舊連結），不可把上傳物件直接丟進照片牆。

```mermaid
flowchart TD
  start[人按儲存照片] --> preview[畫面先顯示新圖、半透明]
  preview --> wait[背景送出]
  wait -->|成功| done[換成後端確認後的圖]
  wait -->|失敗| rollback[還原舊圖、說沒存到]
  rollback --> retry[人再開管理照片再試]
```
...
| 白話（圖上寫的） | 程式對照 |
|------------------|----------|
| 人按儲存照片 | `logActions.handleSavePhotos` |
| 畫面先顯示新圖、半透明 | `buildOptimisticPhotoLinks` → `buildPhotoGrid` |
| 背景送出 | `projectApi.request` action `updateLogPhotosWithUploads` |
| 換成後端確認後的圖 | `_buildLogCard(result.data)` 替換卡片 |
| 還原舊圖、說沒存到 | `restorePhotoGrid` + 人話通知 |
