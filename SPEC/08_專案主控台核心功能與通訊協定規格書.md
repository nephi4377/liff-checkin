...
- **全體發送（必做）**: 勾選後必須幫「每一位在職、權限≥2 的員工」各寫一筆通知。不可把收件人寫成字面 `ALL`，否則智慧通知對不到任何人、看起來像已送出但其實沒人收到。員工名單讀不到時要說失敗，不可顯示已發送。
...
| 白話 | 程式對照 |
|---|----|
| 勾選發送給所有員工 | 前端 `resolveHubStaffRecipientIds`；後端 `_expandNotificationRecipients_` |
| 每位在職員工各一筆通知 | `sendNotification` 寫入 `NotificationCenter`，`RecipientID`＝該員工 LINE UID |
| 名單讀不到就說失敗 | 後端回 `success: false`；前端狀態列人話，不顯示已發送 |
