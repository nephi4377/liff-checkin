...
| 情況 | 人看到什麼 | 系統做什麼 |
|------|------------|------------|
| 一般失敗（例如某次 API 沒過、廠商名冊這次沒載到，但頁面還能用） | 紅字提示；可按「複製錯誤」「開信寄出」「送到 AI」 | 寫入 Logs Explorer（搜尋 `FAIL`）。「送到 AI」才寫進 AI 信箱 |
| 全失敗（畫面「無法繼續」，做不下去） | 再試、複製錯誤、開信、送到 AI；狀態寫「錯誤紀錄產生中」 | 自動進 AI 信箱；錯誤回報開關開時另寄信。三顆按鈕仍可用 |

總開關見下方「日後關閉錯誤回報」。

| 步驟 | 人做什麼 | 畫面呈現 | 結果／下一步 |
|------|----------|----------|----------------|
| 1 開始 | 畫面出現「無法繼續」或送出失敗紅列 | 立刻有再試（若有）、複製錯誤、開信寄出、送到 AI | 系統已在記錄並準備寄信 |
| 2 進行中 | 可按再試、複製、開信，或按「送到 AI」 | 「複製錯誤」一鍵複製；「送到 AI」顯示送出中 | 後端寫入 Logs Explorer，並自動寄到信箱 |
| 3a 成功 | 看提示 | 「錯誤已記錄並寄到 nephihuang@gmail.com」；按鈕變「已寄到信箱」 | 可再試或離開 |
| 3b 寄信沒送出 | 按「複製錯誤」或「開信寄出」 | 人話說明；開信是備案 | 你仍拿得到同一份紀錄 |
| 3c 要給 AI 修 | 按「送到 AI」 | 提示已送到 AI 信箱（雲端有開則另開工） | 不會進你電腦正在開的 Cursor 對話 |
