...
| 情況 | 人看到什麼 | 系統做什麼 |
|------|------------|------------|
| 一般失敗（例如某次 API 沒過、廠商名冊這次沒載到，但頁面還能用） | 紅字提示；可按「複製錯誤」「開信寄出」「複製給 Cursor」 | 寫入 Logs Explorer（搜尋 `FAIL`） |
| 全失敗（畫面「無法繼續」，做不下去） | 再試、複製錯誤、開信、複製給 Cursor；狀態寫「錯誤紀錄產生中」 | 若錯誤回報開關為開：Logs Explorer＋自動寄信。關開關則不回報；三顆按鈕仍可用 |

總開關見下方「日後關閉錯誤回報」。

| 步驟 | 人做什麼 | 畫面呈現 | 結果／下一步 |
|------|----------|----------|----------------|
| 1 開始 | 畫面出現「無法繼續」或送出失敗紅列 | 立刻有再試（若有）、複製錯誤、開信寄出、複製給 Cursor | 系統已在記錄並準備寄信 |
| 2 進行中 | 可按再試、複製、開信，或把「複製給 Cursor」貼回對話 | 「複製錯誤」一鍵複製整份紀錄 | 後端寫入 Logs Explorer，並自動寄到信箱 |
| 3a 成功 | 看提示 | 「錯誤已記錄並寄到 nephihuang@gmail.com」；按鈕變「已寄到信箱」 | 可再試或離開 |
| 3b 寄信沒送出 | 按「複製錯誤」或「開信寄出」 | 人話說明；開信是備案 | 你仍拿得到同一份紀錄 |
| 3c 要給 Cursor 看 | 按「複製給 Cursor」，貼到 Cursor 對話 | 提示已複製 | 網頁**無法**直接傳到 Cursor，一定要貼一次 |
...
使用者說「關掉錯誤回報／不要再自動回報／關除錯」時，**只做這一步**（會計 GAS、帳號 nephihuang）：

1. 在 Apps Script 編輯器執行 `setAccountingErrorReportOnce(false)`（白話：把錯誤回報總開關關掉）。
2. 未設此開關＝**開**（會寫雲端紀錄；全失敗才寄信）。
3. 關掉後：不再自動寫紀錄、不再寄信；畫面上「複製錯誤」**仍可用**。
...
| **`permission`** | Checkin「權限」欄（1–5） | 會計選單／日常 ≥2、主管進階 ≥3、財務頁 ≥4、核准請款 ≥5 | ① URL：`permission`；② `sessionStorage`：`tanxin_operator_v1`（含 legacy `acct_dev_perm`）；③ 正式：每次 `accounting_auth_me` 由後端查表（**登入前清** `acct_emp_*` 再讀表） | 前端身分快取約 12h；若與 HUB URL 權限不一致則強制重驗。後端 `acct_emp_*` 仍可快取，但 **auth_me 當次必重讀** |
...
| **款項待辦摘要** | HUB 側邊欄待審／待匯 | `localStorage` 日快取 `spa_hub_payment_todos_{date}`，**僅成功讀取才寫入**；失敗保留先前內容並顯示「再試一次」，不可把空列表當成沒事。向會計要列表時帶員工編號；**只傳尚未過期的** LINE 通行證，過期則只靠編號查員工表 |
