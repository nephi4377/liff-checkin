...
**收款確認開頁／列表失敗（2026-09-01 巡檢）**：從 LINE「收款確認」進來、身分還沒確認到時，收款區要人話＋「再試一次／重新登入 LINE」，並寫「這不代表沒有收款」。列表第一次讀失敗也一樣：不可停在「—」、不可顯示成沒有收款；有舊列表就留著。本機可用 `?mock=recbootfail`、`?mock=recfail`。
