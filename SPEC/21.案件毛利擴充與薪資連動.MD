...
毛利＝**收入（記帳自動進）− 廠商支出（請款自動進）− 基本費用（有開工日就自動算）**。案件毛利表要能：帶入案件總額（可修正）與收入核對、自動算「基本費用」（基本工資＋五金 1%＋清潔費，畫面低調一筆、後台看細項，**有開工日就算，不等結案**）。廠商勾選只是核對用（沒勾也沒關係）；施工工資由發薪時的案件分攤直接寫入，不另做功能。結案後負責人可從毛利連到薪資申請獎金；權限 5 主管可看每位員工薪資試算；薪資核對「送出」只能在該期最後一天下班時間之後。
...
- **有開工日就計算**（2026-07-26 起）：只要工期天數 > 0 就自動寫入，**不必等結案、不必先有案件總額**。未結案時以「今天」為結束日，每次開詳情頁重算覆寫，金額隨天數增長。
...
2. 基本費用列已計算；checklist 展開**工資／五金／清潔／合計**細項。判定依據＝費用明細真的有備註鍵 `[sys:base_cost]` 的系統支出列（**不是** meta 曾算過就算）；未寫成該列時標籤是「基本費用尚未計算」，唯一常見原因是**未開工**（工期天數 0），此時顯示黃色提醒不算失敗，且**不顯示金額細項**，避免「說已計算卻在明細找不到那一筆」。
