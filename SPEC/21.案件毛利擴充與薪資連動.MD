---
name: 案件毛利擴充與薪資連動
overview: 案件毛利：驗收表廠商／總額核對、基本費用（未結案即時計算、結案固定寫入；工資＋五金1%＋清潔每30萬2000）、施工工資、收支總結；薪資獎金連動、權限5預覽、送出時間閘。
todos:
  - id: e-submit-gate
  ...
  - id: b2-base-cost
    ...
    status: completed
  - id: b1-vendors
    content: 廠商勾選降級為「核對用」：沒勾或未全請款只顯示黃色提醒，不算失敗、不再擴充
    status: completed
  - id: b4-checklist
    content: 案件收支總結：總額vs收入為主要檢查；廠商請款與未開工基本費用為黃色核對項
    status: completed
  - id: b3-labor-wage
    content: 施工人員工資不另做專屬功能：發薪時的案件分攤已直接寫入毛利明細
    status: cancelled
  - id: c-bonus-link
...
毛利＝**收入（記帳自動進）− 廠商支出（請款自動進）− 基本費用（有開工日就自動算）**。案件毛利表要能：帶入案件總額（可修正）與收入核對、自動算「基本費用」（基本工資＋五金 1%＋清潔費，畫面低調一筆、後台看細項；**未結案只做即時計算，結案後才固定寫入毛利明細**）。廠商勾選只是核對用（沒勾也沒關係）；施工工資由發薪時的案件分攤直接寫入，不另做功能。結案後負責人可從毛利連到薪資申請獎金；權限 5 主管可看每位員工薪資試算；薪資核對「送出」只能在該期最後一天下班時間之後。
...
- **基本費用**（併入同一筆低調支出，後台可拆細項）：
  ...
  - 畫面只顯示一筆「基本費用」合計，不強調細項；後台／權限足夠者可展開看三項明細。**未結案時不寫成正式支出列，只顯示即時計算值；已結案才固定寫入毛利明細。**
- **施工人員工資**：發薪時的「案件分攤」直接寫入毛利明細，不另做專屬功能（原 B3 已取消）。
- **廠商全部請款**：核對用黃色提醒——本案勾選的廠商請款單皆已匯款才打勾；沒勾廠商或未全請款不算失敗。
...
- 毛利詳情回傳工期：預設開始＝案場「專案起始日」；**若空白**則與案場「開工日」相同，改用該案 **保護／木作／油漆／系統工程** 第一則「已發布」施工紀錄日（`ProjectLog`，對齊 `resolveProjectStartDateForSiteInfo_`）；結束＝結案日（未結案則暫用今天，僅試算、標「未結案」）。回傳 `duration.start_source`：`manual`／`site`／`project_log`／`none`。
- 儲存主管覆寫的 `工期開始`／`工期結束`（建議放在該案分頁上方 meta 區或總覽擴欄，來源標記 `manual`／`auto`）。（頁面 2026-07-20 起工期改為唯讀顯示，覆寫 API 仍保留。）
...
**工種**（廠商主檔 `trade_category`，共 15 項，來源 `MasterEnums.vendor_trade_categories`）：
...
| 地板 | 木地板、SPC 等 |
| 系統櫃 | 系統櫃體與安裝；舊名「系統櫃/安裝師傅」讀取時自動遷移 |
| 五金 | 滑軌、鉸鏈等 |
| 水電 | 配管配線、廚具設備安裝（舊名「廚衛」讀取時預設遷移為水電） |
| 泥作 | 砌磚、粉刷、防水等 |
| 油漆 | 批土、噴漆、清水模等 |
...
- **有開工日就計算**：只要工期天數 > 0 就先算出目前基本費用，**不必先有案件總額**。未結案時以「今天」為結束日，作為即時計算值；已結案時以結案日為準，並固定寫入毛利明細。
- **開工日來源優先序**（三者任一即算開工）：1. 毛利表**手動指定**的開始日（主管改工期並儲存）→ 2. 案場「專案起始日」→ 3. 第一筆已發布施工紀錄（保護／木作／油漆／系統工程）。
- 案件總額為 0 時：五金＝0、清潔費＝0（公式對 0 總額回 0），之後填「採用總額」再開頁會自動更新。
- 唯一不計算的情況＝還沒開工（工期天數 0），檢查清單顯示黃色「基本費用尚未計算（未開工）」，不算失敗。
- **未結案**：毛利明細**不寫** `基本費用` 系統支出列；詳情頁與檢查清單只顯示 `base_cost_breakdown` 即時計算結果，供看目前毛利。
- **已結案**：毛利明細**只寫一筆**支出：費用類別「基本費用」、來源 `系統`、固定備註鍵（例 `[sys:base_cost]`）去重覆寫；寫入後不再跟著天數浮動。
...
- 工期、總額變更並儲存後重新計算；若已結案則覆寫該系統列，若未結案則只更新即時計算值（不碰其他手動列）。

**B3. 施工人員工資（工務／木作／系統／油漆）— 已取消，不實作**

> 2026-07-26 決定：發薪時的「案件分攤」已直接把工資寫進毛利明細，不需要再做獨立的施工工資功能（舊方式不合用）。以下為原規劃，保留備查。
...
1. 已勾選廠商是否「全部請款完成」——**核對用（`warn_only`）**：沒勾廠商或未全請款都只顯示黃色提醒，不算失敗、不列入「需要注意」。（該廠商＋該案相關請款 `payment_status=已匯款`；無請款列則標「尚無請款」）；UI 依大項目 slot 列出，**未選廠商的 slot 不顯示**；有請款者顯示**匯款日＋金額**（多筆跨月以頓號串列）。
2. 基本費用狀態要分兩種：**未結案**顯示「基本費用即時計算中（未結案）」並展開**工資／五金／清潔／合計**細項，屬黃色提醒，不算失敗；**已結案**則費用明細需真的有備註鍵 `[sys:base_cost]` 的系統支出列，才算「基本費用已計算」。還沒開工（工期天數 0）時顯示黃色「基本費用尚未計算（未開工）」且不顯示金額細項。
...
- **套用**：`margin_apply_bonus` 以 `suggested_amount × ratio / 100` 計算各人金額，寫入該案 meta `bonus_draft`（待薪資審核**對照**）；專案須已結案。
- UI／薪資單用語：**專案報酬**（不加「承攬」）；**只顯示、不併入**本月薪資實發／薪轉；另依 SOP 簽收另付。
- 草稿附 `start_date`（施工／專案起始）、`close_date`（結案日）。
- 對不到員工時允許手動選人（後續 UI）。
...
- Checkin：`payroll_review` `mode=admin_preview`：`permission >= 5` 可傳 `targetUserId`，回傳與本人相同的試算。
- 前端：出勤報表「員工薪資預覽」（僅 ≥5）：選員工 → 選期別 → 顯示試算；可 **代開審**（`manager_submit`）送進會計待審。
...
> **本節狀態**：Phase A 已實作並部署（2026-07-05）；Phase B **已部署（2026-07-06）**（毛利摘要、綁定同步顧客列表、綁定 UX 警告）。
...
| **案件收入審核流** | ✅ 2026-07-06 | 毛利頁登錄「收入」→ `pending_income_review` → 請款審核核准 → LINE 推送客戶 → 客戶① GAS／② 前端防誤觸後單次 GAS 寫入收支；設計師草稿流保留（客戶先確認→財務核准） |

**API 補充（2026-07-06 收入流）**：

- `margin_add_line`（`txn_type=收入`）→ `cfCreateReceiptFromMarginIncome_`；回傳 `review_queue: true`
- `margin_receipt_income_approve`：`pending_income_review` → `pending_customer` + LINE push；legacy `customer_confirmed` 仍寫入毛利
- `margin_receipt_customer_confirm_stage2`：`entry_source=margin_income` 時寫入案件收支並 `approved`
- `official_customer_search`／`line_contact_search` 支援 `fetch_all: true`（前端 24h 名冊快取）
...
**LIFF 設定**：客戶專用 LIFF ID 與 LINE Developers Endpoint URL 見 `modules/accounting/customer-finance-portal.html` 頁內 `CUSTOMER_FINANCE_LIFF_ID`（與員工會計 `policy.liffId` 分開）。外部瀏覽器（含 Cursor 內建）須 `withLoginOnExternalBrowser`；token 過期或後端回「LIFF 驗證失敗」時自動重登一次，畫面另提供「重新登入 LINE」。

**進入方式（2026-08-15）**：LINE 六格文案不變。收款確認那一鍵＝只開收款紀錄；我的案場那一鍵＝只開選材（網址帶選材標記）；綁定新專案那一鍵＝只開綁定。頁內不再放「收款／選材／綁定其他專案」功能卡，也不用頁內返回去切功能。

**測試階段警語（2026-08-16）**：綁定、選材、收款三個畫面標題上方同一則說明（不是錯誤、不擋操作）：目前僅作為參考；自 2026-08-16 開始測試；這天之前的資料不一定齊全。客人仍可查看、確認收款或申請綁定。

**綁定送出失敗（2026-08-30 巡檢）**：按「送出綁定」後若連線失敗或逾時，要人話說明「這不代表沒綁成功、請再試」，按鈕恢復可再按；**不可**停在「處理中，請稍候…」。後端對不上案號／地址／姓名時，仍用原來的人話錯誤。

**選材開頁失敗（2026-08-30 巡檢）**：從 LINE「我的案場」進來、身分還沒確認到時，選材區要人話＋「再試一次」，並寫「這不代表沒有選材資料」。不可只留「載入失敗」，也不可改顯示收款畫面。

**收款確認按下去失敗（2026-08-31 巡檢）**：客人按①「確認此收款紀錄」或②「我確認已收到此收款紀錄」後，連線失敗／逾時要在收款區用人話說「這次沒送到，還沒幫您記下確認，請再按一次」；按鈕恢復可再按。**不可**只在頁頂紅字、也不可錯誤被吞掉看起來像沒按到。①成功後立刻出現②；②成功後立刻寫「已記下您確認收到」，即使列表暫時刷新不到也不要把按鈕變回去。後端狀態對不上時改人話，不要出現「找不到收款」「目前狀態無法第一階段確認」。

客人選材清單讀已登記資料（定案日顯示年月日；照片優先未過期快層，再雲端硬碟縮圖。若只有 Dropbox 分享網址，頁面改成可直接顯示的圖檔網址，不當網頁連結開）。Dropbox 選材夾尚未入列的檔，由**每日自動比對**（未結案）或設計師按「現在補一次」後才出現；**不在客人開頁當場連 Dropbox**（會逾時變成連不上）。開頁約 3–5 秒內先出空間／部位／日期；圖慢到時標「載入中／更新中」。客人看圖快層（約 30 天）與施工回報圖庫分開；Firebase 人怎麼操作見《選材Firebase操作流程》。
...
| 收款列表（唯讀＋待確認操作） | A | 顯示階段、金額、日期、收據編號；待確認項顯示①②。**讀失敗要人話＋再試，不可顯示成沒有收款。** |
...
**員工預覽（唯讀）**：網址加 `#staff_preview=1`（可選 `project_no=案號`；**不要**用 `?`，LINE 回跳不能帶參數）。員工身分（權限 ≥ 2）可下拉切換**全部毛利案號**。**開發測試**（略過登入）同樣列出全部案件。確認／簽名按鈕隱藏，後端亦拒絕寫入。設計師內部頁有「預覽客戶畫面」連結。
...
```
開追加減與收款管理
  → 立刻看到頁面框架；列表區寫「載入案件列表中…」
  → 讀成功且有案件 → 列出案號與待辦徽章
  → 讀成功且沒有案件 → 「尚無案件」
  → 讀失敗：人話＋「再試一次」；有舊名單就留著，不寫成尚無案件
  → 頁上「重新載入最新」可再抓
```

| 白話（圖上寫的） | 程式對照 |
|------------------|----------|
| 開追加減與收款管理 | `modules/accounting/designer-customer-finance.html` 的 `loadOverview`／`loadTodos` |
| 讀失敗可再試、舊名單留下 | 同頁「重新載入最新」、`lastOverviewItems`／`lastTodosItems` |
| 真的沒有案件 | 成功且 `items` 為空才顯示「尚無案件」 |
