...
2. 基本費用列已計算；checklist 展開**工資／五金／清潔／合計**細項。判定依據＝費用明細真的有備註鍵 `[sys:base_cost]` 的系統支出列（**不是** meta 曾算過就算）；未寫成該列時標籤必須是「基本費用尚未計算」並附原因（案件總額為 0／工期天數為 0），且**不顯示金額細項**，避免「說已計算卻在明細找不到那一筆」。
