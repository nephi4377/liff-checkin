...
| 實體 | 工作表／儲存 | 主鍵 | 主要欄位 |
|------|--------------|------|----------|
| **追加減** `contract_adjustment` | accounting-gas 新工作表 `ContractAdjustments` | `adjustment_id` | `project_no`、`type`（追加／減項）、`item_desc`、`amount`（正數）、`status`、`created_by`、`created_at`、`updated_at`、`voided_at`、`void_reason`、`customer_content_confirmed_at`、`customer_signed_at`、`customer_sign_blob_ref`、`company_confirmed_at`、`company_confirmed_by`、`note`、`attachment_ids[]`、`replaces_adjustment_id`（作廢重建鏈） |
...
| 追加減列表（唯讀＋待確認操作） | A | 顯示描述、金額、狀態；待確認項顯示①②按鈕 |
