...
| 日期 | 摘要 |
|------|------|
| 2026-07-12 | **個人假勤全覽**：補未來方向——員工可看全部假勤紀錄＋特休已用／剩餘；細節見 SPEC 17 §4.8 |
| 2026-07-12 | **LINE 只傳圖**：補充未來方向——先入廠商 Drive 資料夾＋快審分類（廢棄／請款／報價／其他）；現行仍靜默 |
...
2. **LINE 只傳圖 → 廠商資料夾＋快審分類**（未來；現行靜默）  
   - **現況（2026-07-11 起）**：進出群／私訊**只傳圖**不建單、不核對卡；請款走「待付款申請」網頁；`#請款` 僅開網頁入口。  
   - **未來目標**：每張圖先**自動存進對應廠商 Drive 資料夾**（對齊上方「廠商檔只維護 Drive」路徑；綁定廠商或 AI 初判工種／廠商）。  
   - **快審 UI**（行政／財務）：一張一張或批次，人工確認分類後才觸發後續——  
     | 分類 | 後續（草案） |
     |------|----------------|
     | **廢棄** | 刪除或移出待處理佇列，不建單 |
     | **請款** | 可選「開待付款申請草稿」或僅歸檔待人工送審 |
     | **報價** | 歸檔至廠商資料夾＋可選通知採購／設計 |
     | **出貨** | 沿用既有 `doc_type` 語意，歸檔 |
     | **其他** | 自填原因；僅歸檔或轉交 |
   - **AI 輔助**：可先建議分類（對齊 `OcrInvoice`／`tryClassifyOpsGroupImageIntent_`），**快審為準**、不自動建請款單。  
   - **開工前**：需與 Drive 廠商夾路徑拍板（§大方向 1）、快審頁放 HUB 或會計模組、佇列表結構。  
   - **不做**：恢復「傳圖即 OCR 核對卡自動建請款」舊路線（已廢止）。

3. **客戶「我的案場」選材／簽約／兩層導覽**  
   2026-07-06 已拍板 IA 與步序；完整細節見下方附錄（尚未實作）。

4. **個人假勤全覽（含特休天數）**（未來；現行不完整）  
   - **現況**：「我的出勤與假勤」有出勤／假勤／薪資核對三 tab；假勤 tab 主要列**申請單**；薪資核對區僅顯示**當期**特休／病假／事假等試算天數；**沒有**全年假勤總覽、特休給額與剩餘。  
   - **目標**：員工可一次看清**全部假勤**——特休、事假、病假、補休、排休等（已核准／待審／退回）；**特休已用天數**（可小數）；若人事有維護給額則顯示**剩餘特休**；可選年度切換（當年＋前 1～2 年唯讀）。  
   - **建議實作**（已寫進 [`17_個人薪資出勤頁計劃書.md`](17_個人薪資出勤頁計劃書.md) **§4.8 Phase A2**）：  
     - Checkin 試算表新分頁 `假勤年度統計`（快取，含 `annualLeaveQuota`／`annualLeaveRemaining`）  
     - API `leave_year_stats`；個人頁新增「**年度統計**」分頁或擴充假勤 tab  
     - 特休天數算法與薪資核對 `stats.annualLeave` **同源**（`Logic_AttendanceReport`）  
   - **開工前**：確認特休給額由誰維護（員工資料欄或人事表）、曆年 vs 週年制顯示規則。  
   - **不做**：在個人頁每次開啟都全表掃打卡＋排班（應讀快取工作頁）。
...
*最後更新：2026-07-12 — 大方向 §4 個人假勤全覽（含特休）；先前 2026-07-11 OneDrive↔Drive、LINE 傳圖。*
