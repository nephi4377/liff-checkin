...
- **狀態格式說明**:
    - `休假`: 此為標準紅色「休」字標籤。
    - `狀態[HH:mm-HH:mm]`: 包含時間註記的假別（如 `特休[08:00-12:00]`），前端會解析括號內文字顯示於下方。
    - `加班`: 在日曆格展示時通常不顯示標籤，但保留於資料中。

**讀失敗 ≠ 沒人休假**：`schedule` 必須是物件才算成功。回傳沒有 `schedule`、或 `success: false`、或連線失敗時，前端不可把空日曆／「大家都上班」當成結果；應人話說明並可再試。真的這個月沒排休＝`schedule` 為空物件 `{}`，才顯示空班表。
