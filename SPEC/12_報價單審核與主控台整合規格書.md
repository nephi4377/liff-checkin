...
*   **報價解析器 Step 0（案號檢查）**：輸入案號後向雲端問「此案是否已有工項」。**讀失敗／逾時不可停在「檢查中」、不可只寫主控台**；畫面要人話＋「再試」。雲端沒連上時**仍可先上傳 Excel 解析**（同步時才再連雲端）。成功才寫「雲端已有／尚無資料」。約 8 秒未回應視為失敗。
...
| 白話（圖上寫的） | 程式對照（開發用） |
|---|----|
| 輸入案號 | `#projectNoInput` 觸發 `verifyProjectNo` |
| 問此案是否已有工項 | Firebase `quotations/{案號}.once('value')`，逾時 8 秒 |
| 人話失敗＋再試 | `#projectVerifyStatus` 文案＋`#btnRetryVerify` |
| 仍可先選檔解析 | 失敗時仍解開 `#dropZone`（同步雲端另一步） |
| 解鎖上傳 | 檢查成功後解開 `#dropZone` |
...
*   **對外／客戶面**：讀同一路徑 Firebase 的頁面（如客戶端）仍須遵守產品 SPEC，**不**在 UI 顯示 `price`、`raw_line`、可辨識之報價細節，見 [13_客戶端唯讀施工進度規格書](./13_客戶端唯讀施工進度規格書.md)。
...
### v3.3 (2026-08-30) - 報價解析器案號檢查失敗可見
- **BudgetWeb Step 0**：雲端檢查失敗／逾時不再停在「檢查中」；人話＋再試；仍可先解析本機 Excel。見 **§2.3**。
...
> 主控台 API 矩陣仍見 `08_專案主控台核心功能與通訊協定規格書.md` §2（僅摘要並連結本節）。
