...
*   **概覽／開案載入（巡檢 2026-08-31）**：第一次讀 Firebase 失敗**不可**繼續顯示骨架或空白卡片、也不可看起來像「沒有案子／沒點到」。無快取時：人話「讀不到」＋再試、自動重試一次；有快取則留舊資料。真的沒有使用中案場才寫「目前沒有使用中的驗收案場」。本機可用 `?mock=dashboard_fail`／`dashboard_empty`／`project_fail` 驗收。
...
### v2.8 (2026-08-31) — 概覽／開案失敗不裝成沒案子
- **全域案場概覽**：讀失敗不再留下骨架；失敗／空／載入中分開；自動再試一次；人話＋再試。
- **開單一案驗收表**：無快取且讀失敗時，輸入區要說「讀不到這案」，不可只剩「輸入案號」像沒點到。
