...
*   **路徑 A (預處理與儲存)**：`BudgetWeb_Standalone` (Excel 解析) -> **Firebase** (`quotations/{projectId}`)。下載的 Context JSON 與上傳至 Firebase 之工項皆含可選欄位 **`price`**；若再次同步雲端且同案號已有舊筆，解析結果 **無** `price` 時**沿用雲端舊值**（避免覆寫進度時一併洗掉金額）。**讀既有雲端失敗時禁止寫入**（人話說明＋可再試），不可把讀失敗當成「沒有舊資料」而整案覆蓋、把驗收進度歸零。
...
### v2.8 (2026-09-01) — 解析器同步讀失敗不覆蓋
- **BudgetWeb 同步**：讀既有 `quotations/{案號}` 失敗時**不寫入**，狀態列人話＋可再試；避免把驗收進度整案歸零卻顯示同步成功。本機 `?mock=syncreadfail` 可驗。見 **§2.3 路徑 A**。
